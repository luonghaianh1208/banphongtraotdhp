...
// Xóa tất cả auto-generated penalties liên quan tới 1 task (dùng khi restore)
const cleanupPenaltiesForTask = async (taskId) => {
  const q = query(
...
// Khôi phục 1 task từ thùng rác + dọn penalty auto liên quan
export const restoreTask = async (taskId) => {
  await updateDoc(doc(db, 'tasks', taskId), {
    isDeleted: false,
    deletedAt: null,
  });
  // BUG-018 FIX: xóa penalty auto-generated để tránh penalty mồ côi
  await cleanupPenaltiesForTask(taskId);
};

// Khôi phục nhiều tasks + dọn penalty auto liên quan
export const restoreTasks = async (taskIds) => {
  ...
  await batch.commit();
  // BUG-018 FIX: dọn penalty cho từng task
  await Promise.allSettled(taskIds.map(id => cleanupPenaltiesForTask(id)));
};
...
export const createPenalty = async (penaltyData) => {
  if (penaltyData.isAutoGenerated && penaltyData.taskId && penaltyData.userId && penaltyData.penaltyTypeId) {
    const compId = `${penaltyData.taskId}_${penaltyData.userId}_${penaltyData.penaltyTypeId}`;
    const penRef = doc(db, 'penalties', compId);

    try {
      await runTransaction(db, async (transaction) => {
        const snap = await transaction.get(penRef);
        if (snap.exists()) {
          throw new Error('ALREADY_EXISTS');
        }
        transaction.set(penRef, {
          ...penaltyData,
          createdAt: new Date().toISOString(),
        ...
        transaction.set(notifRef, {
          ...
          title: 'Giấy phạt vi phạm',
          message: `Bạn bị báo phạt ${formatVND(penaltyData.amount)} do vi phạm ở công việc "${penaltyData.taskTitle}". Vui lòng kiểm tra tab Quản lý Phạt.`,
          type: 'error',
          relatedTaskId: penaltyData.taskId,
          isRead: false,
          createdAt: new Date(),
        });
      });
