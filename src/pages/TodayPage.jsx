...
import {
  isToday, isThisWeek, startOfWeek, endOfWeek, addDays, format,
  startOfMonth, endOfMonth, isSameDay
} from 'date-fns';
import {
  MdAdd, MdToday, MdDateRange, MdList, MdCheckCircle, MdCalendarMonth
} from 'react-icons/md';
...
const TodayPage = () => {
  ...
  const monthGridStart = startOfWeek(mStart, { weekStartsOn: 1 });
  const monthGridEnd = endOfWeek(mEnd, { weekStartsOn: 1 });
  const monthDays = useMemo(() => {
    const arr = [];
    let d = monthGridStart;
    while (d <= monthGridEnd) { arr.push(d); d = addDays(d, 1); }
    return arr;
  }, [monthGridStart.getTime(), monthGridEnd.getTime()]);
  const monthLabel = format(now, 'MM/yyyy');

  // Group tasks by date string
  const groupByDate = (taskList, days) => {
    const map = {};
  ...
  const weekTasks = useMemo(() => activeTasks.filter(task => {
    ...
    return isThisWeek(dl, { weekStartsOn: 1 });
  }), [activeTasks]);
  const tasksByDay = useMemo(() => groupByDate(weekTasks, weekDays), [weekTasks, weekDays]);

  // Month tasks
  const monthTasks = useMemo(() => activeTasks.filter(task => {
    const dl = task.deadline?.toDate ? task.deadline.toDate() : new Date(task.deadline);
    return dl >= monthGridStart && dl <= monthGridEnd;
  }), [activeTasks, monthGridStart, monthGridEnd]);
  const monthTasksByDay = useMemo(() => groupByDate(monthTasks, monthDays), [monthTasks, monthDays]);

  // Personal tasks by date
  ...
  const allSortedTasks = useMemo(() => [...activeTasks].sort((a, b) => {
    ...
    const idxA = order.indexOf(getTaskDisplayStatus(a));
    const idxB = order.indexOf(getTaskDisplayStatus(b));
  ...
  if (isLoading) return (
    <div className="max-w-6xl mx-auto space-y-10 pb-10 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[1,2,3,4].map(i => <div key={i} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-5 h-24 animate-pulse" />)}
      </div>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-fade-in">
        <div className="relative">
          ...
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Việc hôm nay</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-bold uppercase text-[11px] tracking-[0.2em]">Tập trung hoàn thành các mục tiêu quan trọng</p>
        </div>
        {canManageTasks && (
          <button onClick={() => setShowCreate(true)} className="group relative px-8 py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-bold shadow-xl shadow-emerald-500/20 transition-all duration-300 overflow-hidden">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in-up delay-100">
        <StatCard icon={MdList} label="Đang làm" value={activeTasks.length} gradient="from-emerald-500/20 to-emerald-500/5" iconColor="text-emerald-600" />
        <StatCard icon={MdDateRange} label="Cần gấp" value={urgentCount} gradient="from-rose-500/20 to-rose-500/5" iconColor="text-rose-600" />
        <StatCard icon={MdToday} label="Hôm nay" value={activeTasks.filter(t => isToday(t.deadline?.toDate ? t.deadline.toDate() : new Date(t.deadline))).length} gradient="from-sky-500/20 to-sky-500/5" iconColor="text-sky-600" />
        <StatCard icon={MdCheckCircle} label="Đã xong" value={tasks.filter(t => t.isCompleted).length} gradient="from-teal-500/20 to-teal-500/5" iconColor="text-teal-600" />
      </div>
      ...
      <div className="animate-fade-in-up delay-200">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-2 p-1.5 bg-slate-100/80 dark:bg-slate-900/50 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-slate-800 shadow-inner">
            {[
              { value: 'week', icon: MdDateRange, label: `Tuần này (${weekLabel})`, short: 'Tuần' },
              { value: 'month', icon: MdCalendarMonth, label: `Tháng ${monthLabel}`, short: 'Tháng' },
              { value: 'all', icon: MdList, label: 'Tất cả', short: 'Tất cả' },
              ...
              <button key={value} onClick={() => setViewMode(value)}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${viewMode === value
                  ...
                  : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 hover:bg-white/50'}`}>
                <Icon size={18} />
        ...
        {viewMode === 'all' ? (
          allSortedTasks.length === 0 ? (
            <div className="py-12 glass-card"><EmptyState icon={MdToday} title="Không tìm thấy việc nào" message="Có vẻ như bạn đã hoàn thành hết công việc." /></div>
          ) : (
            <div className="space-y-4">
              {allSortedTasks.map((task, idx) => (
                <div key={task.id} className="animate-fade-in-up" style={{ animationDelay: `${(idx+3)*100}ms` }}>
                  <TaskCard task={task} users={users} onClick={setSelectedTask} onApprove={handleApprove} canApprove={canApprove} />
                </div>
              ))}
            </div>
          )
        ...
        {viewMode !== 'all' && (
          <div className="flex items-center gap-6 mt-6 px-2">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded border-l-[3px] border-l-emerald-500 bg-emerald-50" />
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Việc được giao</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded bg-sky-100 border border-sky-300" />
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Việc cá nhân</span>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Modals */}
      <Modal isOpen={showCreate || !!editTask} onClose={() => { setShowCreate(false); setEditTask(null); }} title={editTask ? 'Cập nhật công việc' : 'Tạo công việc mới'} size="lg">
        <TaskForm task={editTask} users={users} currentUser={currentUser} onSubmit={editTask ? (data) => handleEditTask(editTask.id, data) : handleCreateTask} onClose={() => { setShowCreate(false); setEditTask(null); }} />
      </Modal>
      <Modal isOpen={!!selectedTask} onClose={() => setSelectedTask(null)} title="Chi tiết công việc" size="lg">
        <TaskDetail task={selectedTask} users={users} onClose={() => setSelectedTask(null)} onEdit={(t) => { setSelectedTask(null); setEditTask(t); }} />
      </Modal>
    </div>
  );
};
  ...
  <div className="glass-card p-6 group hover:scale-[1.02] transition-all duration-500 relative overflow-hidden border-white/40 dark:border-white/5">
    ...
    <div className="relative z-10">
      <div className="flex items-center gap-3 mb-3">
        <div className={`p-2.5 rounded-xl bg-white/50 dark:bg-slate-800/50 ${iconColor} shadow-sm group-hover:scale-110 transition-transform duration-500`}><Icon size={22} /></div>
